...
as Strands trace attributes, so each Langfuse trace is labelled by conversation
and persona instead of a bare `POST /chat`. Each participant has their own pod,
so this session cache is effectively single-user.
"""
...
# Session-scoped agents keyed by session_id. Value: (agent, token). The agent
# carries the session_id + persona as Langfuse trace attributes, which are fixed
# at construction — so we cache one per session and rebuild if a new login reuses
# the same session id (rare, single-user) with a different persona.
